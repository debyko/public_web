...
function renderInterruptions(S) {
  ...
  const gaps = (Array.isArray(data.gaps) ? data.gaps : []).slice().sort((a, b) => String(b.start || '').localeCompare(String(a.start || '')));
  if (!gaps.length) { el.innerHTML = '<div class="meta" style="padding:14px">No interruption was recorded in this range.</div>'; return; }
  // The API itself caps how many interruptions one response carries and says so; that warning is
  // shown verbatim rather than silently showing a partial list as if it were complete.
  const cutWarning = (data.warnings || []).find(w => /cut|truncat/i.test(String(w)));
  el.innerHTML = (cutWarning ? `<div class="meta" style="padding:8px 14px;border-bottom:1px solid var(--dk-border-hairline)">${esc(cutWarning)}</div>` : '') +
    `<div class="scroll-x scroll-x--tall"><table class="tbl tbl--roomy min-640">
      <thead><tr><th class="l">Venue</th><th>Collector</th><th>Start</th><th>End</th><th>Cause</th></tr></thead>
      <tbody>${gaps.map(g => `<tr>
        <td class="td-venue">${esc(String(g.segment || '').toUpperCase())}</td>
        <td class="td-mono">${esc(g.collector || '—')}</td>
        <td class="td-mono r">${g.start ? esc(utcMinute(g.start)) : '—'}</td>
        <td class="td-mono r">${g.end ? esc(utcMinute(g.end)) : 'ongoing'}</td>
        <td class="td-mono">${g.cause ? esc(g.cause) : '—'}</td>
      </tr>`).join('')}</tbody>
    </table></div>`;
}
